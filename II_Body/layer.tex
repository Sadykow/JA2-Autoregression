Time-series prediction of a neural model relies on an input history of equally distributed samples.
...
That approach is called a stateless model, which only sees dependencies over input samples rather than preserving every received input, like in stateful implementations.
...
\begin{figure}[htbp]
    \centering
    % DST based tests
    \includesvg[width=\linewidth]{II_Body/images/simple_windows.svg}
    \caption{Simple windowing example.}
    \label{fig:windowing_simple}
\end{figure}
\textcolor{blue}{Here, a time-series training set of equally spaced data points is broken up into "windows" of set with, \textit{s}-skip.
A window is defined starting out each data point so that each window overlaps with s-step other windows.
}
\textcolor{blue}{Each window is associated with an output parameter.
The process can be broadly explained in simple terms whereby, prediction using a trained LSTM model of this type will take a window width of input samples and then effectively "choose" the window of best fit from the trained data set, outputting the associated parameter, much like image recognition works. 
}
\textcolor{blue}{In this work, the data set has four features, namely current, voltage, temperature adn SoC, and an output parameter of SoC (i.e the next SoC imediately following the window set.)
The full windowing classification of this data is represented in Figure 2.
For such multi-dimensional arrays, the training/prediction calculation be ... more complicated. }
...
The SoC narrowed between 0 and 1 to represent the percentage charge to two decimals.
