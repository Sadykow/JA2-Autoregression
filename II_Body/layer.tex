...
Due to the size of the windows, equivalent to 8 and a quarter minutes of a discharge process, \textcolor{blue}{with only 1 sample shift \textit{s} between each other}\footnote{\textcolor{blue}{For visualisation purposes, the $s$ shift has been used as 250 seconds, different from the actual implementation. The initial index $i$ was kept as a value close to the beginning of the data, excluding initial data discrepancies}.}, no batching mechanism has been used to reduce computational load and avoid 4-dimensional matrix management.
...
\begin{table}[ht]
    ...
    \begin{tabular}{ l l l }
        ...
        Dropout   & $0.0$ &   \\
        \hline
        Output    & \textcolor{blue}{$activation= \sigma\left(AR, 1 \right)$ }&   \\
        \hline\hline
    \end{tabular}
    }
\end{table}
