...
%
%
\textcolor{red}{In addition to the results of the currently-researched model, Table~\ref{tab:acc-results2} includes the best results from Chapter~\ref{cha:Analysis} LSTM models, which are considered as the two best models for charge- discharge SoC estimation on other profiles.}
\ifthenelse{\boolean{thesis}}{Even though a three-layer model was proven superior to a single one in the previous chapter, the same amount of neurons in one layer but with autoregression has shown better average results than any previously-reported variations.}
{\textcolor{red}{Even though a three-layer model was proven superior to a single one in the earlier works~\ref{}}, the same amount of neurons in one layer but with autoregression has shown better average results than any previously-reported variations.}
